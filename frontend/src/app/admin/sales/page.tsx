...
export default function SalesPage() {
  ...
  const columns: ColumnDef<Sale>[] = [
    ...
    { header: "Sold By", accessorKey: "sold_by_name" },
    {
      header: "Date",
      accessorKey: "sold_at",
    ...
    <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-1">
          ...
          <input name="sale_price" type="number" step="0.01" required className="flex h-9 w-full rounded-md border border-slate-200 px-3 py-1 text-sm" />
        </div>
        <div className="space-y-1">
          <label className="text-sm font-medium">Customer Name *</label>
          <input name="customer_name" required className="flex h-9 w-full rounded-md border border-slate-200 px-3 py-1 text-sm" />
        </div>
        <div className="space-y-1">
