    ...
    <div className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop py-3xl">
      ...
      <div className="flex flex-col lg:flex-row gap-gutter">
        <div className="flex-1 space-y-md">
          {items.map((item) => (
            <div key={item.id} className="flex items-start gap-md p-lg bg-white rounded-xl border border-outline-variant">
              ...
              <div className="flex-1 min-w-0 flex flex-col gap-2">
                <div className="flex items-start justify-between gap-2">
                  <Link href={`/products/${item.id}`} className="font-title-lg text-title-lg hover:text-primary transition-colors line-clamp-1 flex-1 min-w-0">
                    {item.name}
                  </Link>
                  <button
                    onClick={() => removeItem(item.id)}
                    className="text-on-surface-variant hover:text-error transition-colors flex-shrink-0 min-w-[36px] min-h-[36px] flex items-center justify-center"
                  >
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center border border-outline-variant rounded-lg flex-shrink-0">
                    <button
